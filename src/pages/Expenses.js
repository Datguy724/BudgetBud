...
function ExpensesPage() {
  ...
  const handleAddExpense = () => {
    const newExpense = {
      ...
      amount: Number.parseFloat(expenseInput.amount) || 0,
    ...
    <div className="expenses-page">
      <main className="main-content">
          ...
          {expenses.map((exp) => (
            <div className="expense-row" key={exp.id}>
              ...
              <div className="expense-cell">
                <button className="edit-btn" onClick={() => handleDeleteExpense(exp.id)}>
                  Delete
                </button>
              </div>
            </div>
        ...
        <div className="expense-form">
          <input name="date" type="date" placeholder="Date" value={expenseInput.date} onChange={handleChange} />
          <input name="name" type="text" placeholder="Expense Name" value={expenseInput.name} onChange={handleChange} />
          <input name="amount" type="number" placeholder="Amount" value={expenseInput.amount} onChange={handleChange} />
          <input
            name="category"
            type="text"
